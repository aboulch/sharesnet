...
--
-- It is based on the code from Facebook, Inc
-- at https://github.com/facebook/fb.resnet.torch
------------------------------------
------------------------------------
...
local function createModel(opt)
   ...
   -- CIFAR networks
   local function basicblock(n, stride, conv_ref)
      ...
      s:add(SBatchNorm(n))
      s:add(ReLU(true))
      s:add(share_convs[#share_convs]:clone('weight','bias', 'gradWeight','gradBias'))
      s:add(SBatchNorm(n))
   ...
   local function bottleneck(n, stride, conv_ref)
      local nInputPlane = iChannels
      iChannels = n * 4
   ...
   local function layer(block, features, count, stride)
      ...
      -- create the convolution for sharing weights
      share_convs[#share_convs+1] = Convolution(features,features,3,3,1,1,1,1)
